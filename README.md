...
Agentify Code builds a rich, structured prompt from your workspace (files, git status, errors, open tabs) and copies it to your clipboard. Paste it into any AI app you already have access to, like Microsoft Copilot, ChatGPT, Claude, Gemini, or anything else. Paste the response back and the extension parses it into individual actions (file edits, new files, shell commands) that you approve one by one before anything runs.
...
4. **Approve actions** - each suggested file change or command shows as a card, approve or skip individually
...
4. Press **F5** and a new VS Code window opens with the extension loaded

## Usage

### Opening the panel

- Click the **Agentify Code icon** in the Activity Bar on the left sidebar
...
- Toggle **Compact mode** for large codebases, sends file tree and git diff only, skips open tabs
- Click **Build Prompt** and your workspace context is scanned and copied to clipboard automatically
...
MIT, Copyright (c) 2026 Alex Zimmermann
